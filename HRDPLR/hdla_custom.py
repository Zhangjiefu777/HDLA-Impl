...
class HDLA_Custom(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # x: b n d
        # linear map
        q = self.q_proj(x) # [B, T, K]
        k = self.k_proj(x) # [B, T, K]
        v = self.v_proj(x) # [B, T, V]
        if self.use_offset:
            f = self.f_proj(x) + self.offset
        else:
            f = self.f_proj(x) # [B, T, K]
        ...
        """
        The decay of HDLA is:
        H 
        = (I - beta * k @ k.T) @ Diag(f) @ (I - beta * k @ k.T)
        = [I - (beta * k) @ k.T] @ [Diag(f) - (beta * f * k) @ k.T]
        ...
        The resulting Diagonal-Plus Rank-2 decay structure is:
