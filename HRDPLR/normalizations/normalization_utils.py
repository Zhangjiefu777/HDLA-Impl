...
HDLA_DEBUG = eval(os.getenv("HDLA_DEBUG", "False"))
...
def _initialize_weights(self, module):
    ...
    if self.token_mixer_init_type == 0:
        ...
        if hasattr(module, "static_w") and module.static_w is not None:
            nn.init.ones_(module.static_w)
    ...
    if self.rescale_type == 1:
        ...
        for name, p in module.named_parameters():
            if name in ["o_proj.weight", "out_proj.weight"]:
                ...
                with torch.no_grad():
                    p /= math.sqrt(num_residuals_per_layer * self.num_hidden_layers)
...
def _post_init_weights(
    ...
    for layer in self.model.layers:
        ...
        if hasattr(layer.token_mixer, "q_norm"):
            try:
                layer.token_mixer.q_norm._init_weights()
            except:
                pass
        if hasattr(layer.token_mixer, "k_norm"):
            try:
                layer.token_mixer.k_norm._init_weights()
            except:
                pass
